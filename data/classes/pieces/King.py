...
class King(Piece):
				...
				if queenside_rook != None:
					if not queenside_rook.has_moved:
						if [
							...
							castlingrights = castlingrights | 0b01
					else:
				...
				if kingside_rook != None:
					if not kingside_rook.has_moved:
						if [
							...
							castlingrights = castlingrights | 0b10
					else:
	...
	def get_valid_moves(self, board):
		...
		if not board.is_in_check(self.color):
			if self.can_castle(board) == 0b01 or self.can_castle(board) == 0b11 :
				output.append(
					board.get_square_from_pos((self.x - 2, self.y))
				)
			if self.can_castle(board) == 0b10 or self.can_castle(board) == 0b11:
				output.append(
					board.get_square_from_pos((self.x + 2, self.y))
				)
