...
class Pawn(Piece):
	...
	def get_moves(self, board, checkEP = True):
		output = []
		...
		if self.color == 'white':
			if self.x + 1 < 8 and self.y - 1 >= 0:
				...
				#correct code move to other pieces. 
				if (square.occupying_piece != None and square.occupying_piece.color != self.color)  or ((square.coord == board.epsquare) and checkEP) :
					if checkEP or square.occupying_piece.color != self.color :
						output.append(square)
	...
	def attacking_squares(self, board, checkEP = False):
		moves = self.get_moves(board, checkEP)
