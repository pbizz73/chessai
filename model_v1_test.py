...
for i in range (1000,2000):
    ...
    item2 = dataset.__getitemorg__(i)
    trueEval = item['eval']
    bitBoardInfo = item['binary']
    t =torch.from_numpy(bitBoardInfo)
    predEval = model(t).item()
    print( "The true evaluation is " + str(trueEval) +", the predicted eval is " + str(predEval) + ". Fen String:" + str(item2.fen))
